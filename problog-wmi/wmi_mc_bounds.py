...
class Interval(object):
    ...
    def is_valid(self):
        try:
            return sympy.eval(self.bounds[0] < self.bounds[1])
        except Exception:
            pass
        return True
...
class Intervals(object):
    ...
    def create(cls, string):
        ...
        try:
            ...
            bounds = parse_expr(match.group(2)), parse_expr(match.group(3))
...
class Element(object):
    ...
    def integrate(self, operator):
        ...
        for (i, f) in self.list:
            bounds = i.bounds()
            if not i.is_valid() or f.is_null() | any(map(lambda (b1, b2): b1 == b2, bounds)):
                ...
                def sampler():
                    print(bounds)
                    while True:
                        x0 = sympy.sympify(random.uniform(bounds[0][0], bounds[0][1]))
                        y0 = sympy.sympify(random.uniform(bounds[1][0], bounds[1][1]))
                        x1 = 0
                        b1 = bounds[2][0]
                        b2 = bounds[2][1]
