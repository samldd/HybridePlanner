...
void CSolver::set_var_weight(vector<double> * weight)
{
	...
	for (int i = variables().size()-1; i >= 1; --i)
	{
		...
		if (var.pos_weight == var.neg_weight)
			var.internal = true;	// add a tag for a component with only interal nodes
		else
			var.internal = false;	// an internal node has a weight sum of 2
	}
}
